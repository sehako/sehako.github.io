...
함수 사용시에는 헨들바 파일에 `{{헬퍼 함수 이름 변수1, 2, ...}}`로 사용하면 된다. 헬퍼 함수를 중첩해서 사용하는 경우에는 `{{헬퍼 함수1 (헬퍼 함수2 변수1 변수2) 변수3}}`처럼 사용하면 된다. 이제 `app.js`에 핸들파 지정 함수 부분을 수정한다.
...
`create`로 핸들파 객체를 만들 때 옵션으로 헬퍼 함수를 추가할 수 있다.
...
리스트를 가져오는 건 `services/post-service.js`에 구현한다.
...
내장함수를 사용할때는 `{{#function}}` 사용이 끝나면 `{{/function}}`을 표기해줘야한다.
