...
![그래프 종류](/assets/images/algorithm/24-12-07-graph/01.png)
...
|     구분     |                                          그래프                                           |                                        트리                                         |
| :----------: | :---------------------------------------------------------------------------------------: | :---------------------------------------------------------------------------------: |
|     정의     |          노드(node)와 그 노드를 연결하는 간선(edge)을 하나로 모아 놓은 자료 구조          | 그래프의 종류<br>DAG(Directed Acyclic Graph, 방향성이 있는 비순환 그래프)의 한 종류 |
|    방향성    |                방향그래프(Directed)<br>무방향 그래프(Undirected) 모두 존재                |                             방향 그래프(Directed Graph)                             |
|    사이클    | 사이클(Cycle) 가능<br>자기참조(self-loop) 가능<br>순환(Cyclic), 비순환(Acyclic) 모두 존재 | 사이클(Cycle) 불가능<br>자기참조(self-loop) 불가능<br>비순환 그래프(Acyclic Graph)  |
|  루트 노드   |                                   루트 노드의 개념 없음                                   |         한 개의 루트 노드 존재<br>모든 자식 노드는 한 개의 부모 노드만 가짐         |
|  부모-자식   |                                    부모-자식 개념 없음                                    |               부모-자식 관계 존재<br>top-bottom 또는 bottom-top 구조                |
|     모델     |                                       네트워크 모델                                       |                                      계층 모델                                      |
|     순회     |                                         DFS, BFS                                          |                          DFS, BFS의 Pre-, In-, Post-order                           |
|  간선의 수   |                   그래프에 따라 간선 수가 다름<br>간선이 없을 수도 있음                   |                           노드가 N개면 항상 N-1개의 간선                            |
|     경로     |                                             -                                             |                            임의의 두 노드 간 경로는 유일                            |
| 예시 및 종류 |      지도, 지하철 노선도, 최단 경로<br>전기 회로, 도로(교차점·일방통행)<br>선수 과목      |    이진 트리, 이진 탐색 트리<br>균형 트리(AVL, Red-Black)<br>이진 힙(최대·최소)     |
...
<p align="center">
    <img src="/assets/images/algorithm/24-12-07-graph/02.png" width="30%" />
</p>
...
<p align="center">
    <img src="/assets/images/algorithm/24-12-07-graph/03.png" alt="방향 그래프" width="30%" />
</p>
