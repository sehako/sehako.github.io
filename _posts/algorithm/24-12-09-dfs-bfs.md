...
<p align="center">
    <img src="/assets/images/24-12-09-dfs-bfs/01.png" alt="tree" width="30%" />
</p>
...
|   구현 방식   |         재귀 또는 스택          |                 큐                  |
|  데이터 구조  |  스택 (명시적 또는 호출 스택)   |                 큐                  |
| 메모리 사용량 |         일반적으로 적음         | 너비가 넓은 경우 메모리 사용량 증가 |
|   경로 탐색   |      특정 경로 탐색에 유리      |        최단 경로 탐색에 유리        |
|  적합한 문제  |    미로 찾기, 모든 경로 탐색    |      최단 거리 문제, 레벨 탐색      |
|  시간 복잡도  |            O(V + E)             |              O(V + E)               |
| 메모리 복잡도 |              O(V)               |                O(V)                 |
|     장점      | 메모리 효율적, 경로 탐색에 유리 |   최단 거리 보장, 레벨 탐색 가능    |
|     단점      |     최단 경로 보장이 안 됨      |   메모리 사용량이 많아질 수 있음    |
