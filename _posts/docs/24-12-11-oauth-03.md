...
아무튼 이런식으로 만들면 서비스에서 `ProviderFactory`를 의존할 필요가 없어진다. 또한 (아마도) 로깅 처리 때 다뤘던 생성자 주입을 사용하면 `ProviderFactory`를 정적 메소드로 활용하여 `ArgumentResolver`에서도 의존성 주입을 할 필요가 없을 것 같다. 뭐가 더 나은지는 모르겠다...
