...
> _Amazon Simple Storage Service(S3)는 업계 최고 수준의 확장성, 데이터 가용성, 보안 및 성능을 제공하는 객체 스토리지 서비스입니다._ - AWS
...
![지불 정보](/assets/images/infrastructure/25-02-25-deploy-frontend/01.png)
...
![생성된 버킷 정보](/assets/images/infrastructure/25-02-25-deploy-frontend/02.png)
...
![업로드 사진](/assets/images/infrastructure/25-02-25-deploy-frontend/03.png)
...
![디렉터리 선택](/assets/images/infrastructure/25-02-25-deploy-frontend/04.png)

이대로 업로드를 클릭하면 버킷에 build 디렉터리를 포함한 하위 파일들이 모두 업로드할 수 있다.

![업로드 결과](/assets/images/infrastructure/25-02-25-deploy-frontend/05.png)
...
> _특정 요청에 대한 동적 처리 가능 (예: URL 리다이렉트, 헤더 수정)_
...
![배포 생성](/assets/images/infrastructure/25-02-25-deploy-frontend/06.png)
...
![옵션 1](/assets/images/infrastructure/25-02-25-deploy-frontend/07.png)

![옵션 2](/assets/images/infrastructure/25-02-25-deploy-frontend/08.png)

![옵션 3](/assets/images/infrastructure/25-02-25-deploy-frontend/09.png)
...
![루트 오브젝트 선택](/assets/images/infrastructure/25-02-25-deploy-frontend/10.png)

**원본 탭**

이제 만들어진 배포에서 원본 탭으로 이동하면 하나의 원본이 보일 것이다 이를 체크하면 편집할 수 있다.

![원본 영역](/assets/images/infrastructure/25-02-25-deploy-frontend/11.png)

![원본 설정](/assets/images/infrastructure/25-02-25-deploy-frontend/12.png)
...
![버킷 정책 설정](/assets/images/infrastructure/25-02-25-deploy-frontend/13.png)

이제 모든 설정이 완료되었다. 일반 탭의 세부 정보 부분에서 배포 도메인 이름을 복사해서 접속하면 나의 경우 리액트의 기본 페이지가 나올 것이다.

![배포 결과](/assets/images/infrastructure/25-02-25-deploy-frontend/14.png)
...
![도메인 구매 결과](/assets/images/infrastructure/25-02-25-deploy-frontend/15.png)

## Route 53 호스팅 영역 생성

이제 Route 53 서비스로 가도록 하자. Route 53으로 이동하면 호스팅 영역 생성이라는 버튼이 존재한다. 이를 클릭하면 다음과 같은 설정 부분이 나올 것이다.

![호스팅 영역 생성](/assets/images/infrastructure/25-02-25-deploy-frontend/16.png)
...
![네임 서버 설정](/assets/images/infrastructure/25-02-25-deploy-frontend/17.png)

![네임 서버 설정 결과](/assets/images/infrastructure/25-02-25-deploy-frontend/18.png)
...
![인증서 발급 부분](/assets/images/infrastructure/25-02-25-deploy-frontend/19.png)

여기서 그냥 내가 구매한 도메인을 입력하면 된다. 그리고 인증서가 발급될 떄 까지 기다려야 한다. 제대로 설정을 했다면 10분 정도 걸릴 것이다.

![인증서 발급 완료](/assets/images/infrastructure/25-02-25-deploy-frontend/20.png)
...
![대체 도메인 설정](/assets/images/infrastructure/25-02-25-deploy-frontend/21.png)

그럼 설정 부분이 다음과 같이 될 것이다.

![대체 도메인 설정 완료](/assets/images/infrastructure/25-02-25-deploy-frontend/22.png)

## 도메인 연결

이제 다시 Route 53으로 돌아가서 레코드 유형을 A로 하여 별칭을 클릭하고 다음과 같이 설정하면 된다.

![Route 53 도메인 연결](/assets/images/infrastructure/25-02-25-deploy-frontend/23.png)

이제 최종적으로 sehako.store에 접속하면 다음과 같은 화면이 나오게 된다.

![도메인 연결 결과](/assets/images/infrastructure/25-02-25-deploy-frontend/24.png)
