...
![웹 서버 시장 점유율](/assets/images/infrastructure/25-02-22-nginx/01.png)

이런 아파치 웹 서버는 클라이언트의 요청에 대해서 각각의 스레드를 생성하여 처리한다. 이런 처리 방식은 컴퓨터의 보급률이 올라감에 따라 클라이언트의 요청이 증가하면서 클라이언트가 1만 이상 동시에 요청을 보낸다면 CPU와 메모리의 사용량이 증가하고 문맥 전환에 대한 비용이 커지면서 결과적으로 처리 가능한 클라이언트의 초당 요청 수(RPS)가 감소하게 되는 C10K 문제가 발생한다. 즉, 아파치 웹 서버는 대규모 동시 접속자 수를 효율적으로 처리할 수 없는 구조적 한계를 가지고 있다.
...
![서버 구조](/assets/images/infrastructure/25-02-22-nginx/02.png)
