...
스케줄러 작업 실행 로그를 살펴보면 JVM 설정인 4기가 한계 기준 최대 30프로 이상의 메모리를 점유하고, 20분에 가까운 시간이 소요되는 것을 확인할 수 있다.
...
100만 건의 데이터를 대상으로 메모리가 2% 이하로 개선되었고, 시간도 5분 가량 더 빨라진 것을 확인할 수 있다. 하지만 이렇게 하면 과연 끝일까? 
...
정리하자면 Spring Scheduler에서 배치 작업을 처리하려면 비즈니스 로직 뿐 아니라 여러 예외 상황에 따른 처리를 해줘야 한다. 단점을 정리해보면 다음과 같다.
