...
그렇다면 SPA 애플리케이션이라고 가정한다면, 어떻게 할 수 있을까? 사실 전체적인 과정은 앞서 백엔드 주도 방식과 별 다를 게 없다. 단지 마지막에 백엔드에서 OAuth 2.0 제공자로부터 리소스를 얻은 다음에 SPA로 리다이렉트할 때 JWT 엑세스 토큰 대신 서비스에서 자체 발급한 쿠키를 전달하기만 하면된다.

이후 JWT 토큰은 BFF 서버에서 보관하다가 자체 발급된 쿠키를 받으면 BFF에서 Authorization 헤더에 JWT 토큰을 담아서 요청을 전달하는 구조다.
...
다음 포스팅은 스프링 시큐리티를 활용하여 SPA 환경에서 백엔드 주도 방식의 OAuth 2.0 로그인을 구현하는 방법을 알아보도록 하자.
